...
function decision_tree_classifier()
  Random.seed!(0)
  n = 400
  X = 2 * rand(n, 2)
  ...
  for max_depth = 1:4, spl in [:best, :random]
    model = DecisionTreeClassifier(max_depth=max_depth, splitter=spl)
    fit!(model, X, y)
    ...
    title!("DecisionTreeClassifier: acc = $(accuracy_score(y, y_pred)), max_depth=$max_depth, splitter=$spl")

    png("decision-tree-classifier-$k")
    k += 1
  end
end
